...
```typescript
const userId = session?.user?.id || req.ip || req.headers.get('x-forwarded-for') || "anonymous";
```
...
export function assertOwnership(data: { userId?: string } | undefined, userId: string): void {
